...
void oslobodi(PCB* t){
	delete t->mySignalContainter;
		t->mySignalContainter=NULL;
		delete t->myQ;
		t->myQ=NULL;
		delete [] t->myBlockedSig;
		t->myBlockedSig=NULL;
		t->myParent=0;
		t->myThread=NULL;
		delete[] t->stack;
	...
	if(running->waitRet!=-3&&contextLock!=1){//contextLock da ne zovem sam sebe
			...
			while(tmp1<tmp)
			{
				...
				if(tmp2!=-1&&GSignalBlock[tmp2]!=1&&running->myBlockedSig[tmp2]!=1){
					...
					if(tmp2!=-1)running->myQ->addQ(tmp2);
				}
				tmp--;
			}
			if(running->finished==1)
...
void interrupt timer(){
	...
	if(csd==0){//okinuo se timer
		...
		asm{
			...
			if(csd==3){
				while(running!=NULL&&running->myThread==target){
					running = Scheduler::get();
				}
			}
			else{
			...
			if(csd==3){//da li je usledio nasilan prekid usled signala 0
				...
				if(target!=NULL){
					parent = Thread::getThreadById(target->myPCB->myParent);
									...
									target->myPCB->started=2;
									target->myPCB->myThread->signal(2);
									target->myPCB->started=1;
									oslobodi(target->myPCB);
									//target->myPCB=NULL;
									csd=1;
				}
